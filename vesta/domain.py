...
def derive(
    repo: Path | str, model: Optional[str] = None, rebuild: bool = False
) -> Optional[Ontology]:
    """Ask Metis what the work in this repository is.

    Expensive and slow — it is a model reading a project and naming what it
    does — so it is derived once and kept. Returns what is already held unless
    `rebuild`.
    """
    import asyncio
    ...
    said = _speaks(root)
    if len(said) < 3:
        logger.info("%s says too little about itself to derive an ontology", root)
        return None
    ...
    try:
        ...
        logger.info("metis is not installed, so no ontology can be derived: %s", exc)
        return None

    purpose = _purpose_from(root, said)
    grounding = "\n\n".join(f"{where}: {text}" for where, text in said)
    ...
    async def run():
        return await analyze_async(
            ...
            grounding=grounding,
            model=model,
            working_dir=VESTA_HOME / "cache",
            use_embeddings=False,
        )

    try:
        document = asyncio.run(run())
    except Exception as exc:  # noqa: BLE001 - a failed derivation is not fatal
        logger.info("could not derive an ontology for %s: %s", root, exc)
        return None
    ...
    found = Ontology(
        ...
        derived_at=__import__("time").time(),
    )

    try:
        _where(root).write_text(found.model_dump_json(), encoding="utf-8")
    except OSError as exc:
        logger.info("could not keep the ontology: %s", exc)
